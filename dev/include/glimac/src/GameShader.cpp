...
namespace glimac
{
	...
	bool gameShader::init(const FilePath* vsFilename, const FilePath* fsFilename)
	{
		std::cout << std::endl <<"**********************************************************"<< std::endl 
		<< "Initialisation gameShader" << std::endl;

		glimac::FilePath vsPath, fsPath;
		vsPath = "./src/main/shaders/shader.vs.glsl";
		fsPath = "./src/main/shaders/shader.fs.glsl";

		FilePath applicationPath("./src/main");
		Program program = loadProgram(
        applicationPath.dirPath() + "shaders/shader.vs.glsl",
        applicationPath.dirPath() + "shaders/shader.fs.glsl"
    	);

/*
		_Program = loadProgram(*vsFilename,*fsFilename);
*/
		use();
		...
		std::cout << "Fin initialisation gameShader" << std::endl;
		std::cout << "**********************************************************"<< std::endl << std::endl;
	...
	void gameShader::setTextureLocation(unsigned int v)
	{
	  glUniform1i(_Texlocation, v);
	}

	void gameShader::setProjectionMatrix(const float* v)
	{
		glUniformMatrix4fv(_PMlocation, 1, GL_FALSE, v);
	}

	void gameShader::setWorldMatrix(const float* v)
	{
		glUniformMatrix4fv(_WMlocation, 1, GL_FALSE, v);
	}

	void gameShader::setNormalMatrix(const float* v)
	{
		glUniformMatrix4fv(_NMlocation, 1, GL_FALSE, v);
	}

	void gameShader::setDiffuse(const glm::vec3 d)
	{
		glUniform3f(_diffuseLocation, d.r, d.g, d.b);
	}

	void gameShader::setSpecular(const glm::vec3 s)
	{
		glUniform3f(_specularLocation, s.r, s.g, s.b);
	}

	void gameShader::setShininess(const float s)
	{
		glUniform1f(_shininessLocation, s);
	}


	void gameShader::setLights(const std::vector<Light*> lights, const glm::mat4* ViewMatrix) {
		GLuint typeLocation;
