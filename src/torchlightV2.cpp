...
*/
    ...
    std::unique_ptr<Image> ptr_terre = loadImage("/home/benjamin/Documents/openGL/S3/projet/include/assets/EarthMap.jpg");
    if (ptr_terre == NULL)
     std::cout << "Terre non chargé" << '\n';
    else
     std::cout << "Terre chargé" << '\n';
    ...
    std::unique_ptr<Image> ptr_nuages = loadImage("/home/benjamin/Documents/openGL/S3/projet/include/assets/CloudMap.jpg");
    if (ptr_nuages == NULL)
    std::cout << "Nuages non chargé" << '\n';
    else
    std::cout << "Nuage chargé" << '\n';
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ptr_terre->getWidth(), ptr_terre->getHeight(), 0, GL_RGBA, GL_FLOAT, ptr_terre->getPixels());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // appliquer un filtre linéaire
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0);

    GLuint textureNuages;
    glGenTextures(1, &textureNuages);
    glBindTexture(GL_TEXTURE_2D, textureNuages);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ptr_nuages->getWidth(), ptr_nuages->getHeight(), 0, GL_RGBA, GL_FLOAT, ptr_nuages->getPixels());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // appliquer un filtre linéaire
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glBindTexture(GL_TEXTURE_2D, 0);
    ...
    while(!done) {
        ...
        if(windowManager.isMouseButtonPressed(SDL_BUTTON_WHEELUP)) Camera.moveFront(0.01);
        else if(windowManager.isMouseButtonPressed(SDL_BUTTON_WHEELDOWN)) Camera.moveFront(-0.01);
...
*/
        ...
        for (size_t i = 0; i < nbLune; i++) {

          glm::mat4 moonMVMatrix = glm::translate(glm::mat4(1), glm::vec3(0, 0, -5));
          moonMVMatrix = glm::rotate(moonMVMatrix, windowManager.getTime(), AxesRotation[i]);
          moonMVMatrix = glm::translate(moonMVMatrix, AxesTranslation[i]); // Translation * Rotation * Translation
          moonMVMatrix = ViewMatrix * glm::scale(moonMVMatrix, glm::vec3(0.2, 0.2, 0.2));
